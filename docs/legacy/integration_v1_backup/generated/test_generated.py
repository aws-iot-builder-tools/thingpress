...
def setup_test_environment():
    ...
    try:
        ...
        # Fall back to subprocess if the import doesn't work
        logger.info("Using subprocess to generate certificates")
        subprocess.run([
            'python', 
            os.path.join(os.path.dirname(__file__), '../../../src/certificate_generator/generate_certificates.py'),
            '--count', '5',
            '--output-dir', temp_dir,
            '--batch-size', '5'
        ], check=True)
    
    # Find the generated certificate file
    cert_files = [f for f in os.listdir(temp_dir) if f.startswith('certificates_') and f.endswith('.txt')]
    if not cert_files:
        raise Exception("No certificate files were generated")
    
    # Upload the certificate file to S3
    s3 = boto3.client('s3')
    cert_file_path = os.path.join(temp_dir, cert_files[0])
    cert_key = f"test-certificates/generated-{test_id}.txt"
    
    with open(cert_file_path, 'rb') as f:
        s3.upload_fileobj(f, input_bucket, cert_key)
...
def run_generated_test(env):
    """Run the Generated Certificates provider test."""
    metrics = TestMetrics("generated_integration_test")
    cleanup = ResourceCleanup()
    
    # Add the certificate file to cleanup
    cleanup.add_s3_object(env['input_bucket'], env['cert_key'])
    
    try:
        # Step 1: Trigger the Generated Certificates provider
        ...
        provider_context = type('obj', (object,), {
            'function_name': 'test_generated_provider',
        ...
        try:
            ...
            if os.path.exists(env['temp_dir']):
                for file in os.listdir(env['temp_dir']):
                    os.remove(os.path.join(env['temp_dir'], file))
                os.rmdir(env['temp_dir'])
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Run the test
        test_results = run_generated_test(env)
