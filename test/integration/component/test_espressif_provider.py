"""
Espressif Provider Component Integration Test

Tests the deployed Espressif provider Lambda function through its interface:
1. Invokes the provider function with a test CSV manifest
2. Verifies it processes the CSV correctly
...
class EspressifProviderComponentTest(ProviderComponentTest):
    """Component test for Espressif provider"""
    
    def __init__(self):
        super().__init__('espressif')
        self.test_manifest_path = project_root / 'test/artifacts/manifest-espressif.csv'
        
    def run_test(self) -> dict:
        """Run the complete Espressif provider component test"""
        ...
        try:
            ...
            step2 = self.log_step("upload_manifest", "Upload Espressif CSV manifest to S3")
            manifest_key = f"component-test/{self.test_id}/manifest.csv"
            ...
            step3 = self.log_step("invoke_provider", "Invoke Espressif provider function")
            ...
            # Step 5: Validate CSV parsing results
            step5 = self.log_step("validate_csv_parsing", "Validate CSV was parsed correctly")
            
            # Check that certificates have expected Espressif format
            if certificates:
            ...
            # For Espressif, thing names are typically derived from certificate CN
...
def run_espressif_component_test():
    """Run the Espressif provider component test"""
    test = EspressifProviderComponentTest()
    results = test.run_test()
    
    # Print summary
    print("\n" + "="*60)
    print(f"🧪 ESPRESSIF PROVIDER COMPONENT TEST RESULTS")
    print("="*60)
...
if __name__ == "__main__":
    success = run_espressif_component_test()
