...
class AWSHelpers:
                    ...
                    for event in events_response['events']:
                        if request_id in event['message']:
                            if 'END RequestId' in event['message']:
                                ...
                                logger.warning("Lambda execution may have failed: %s",
                                               event['message'])

            except ClientError as e:
                logger.debug("Error checking logs: %s", e)

            time.sleep(2)

        logger.warning("Timeout waiting for Lambda %s completion", function_name)
        return False
    ...
    def get_recent_iot_things(minutes: int = 10, region: str = 'us-east-1') -> list[dict]:
        ...
        try:
            ...
            logger.error("Failed to get recent IoT things: %s", e)
            return []
    ...
    def get_thing_certificates(thing_name: str, region: str = 'us-east-1') -> list[str]:
        ...
        try:
            ...
            logger.error("Failed to get certificates for thing %s: %s", thing_name, e)
            return []
    ...
    def get_certificate_policies(certificate_arn: str, region: str = 'us-east-1') -> list[str]:
        ...
        try:
            ...
            logger.error("Failed to get policies for certificate %s: %s", certificate_arn, e)
            return []
    ...
    def verify_s3_object_exists(bucket: str, key: str, region: str = 'us-east-1') -> bool:
        ...
        try:
            ...
            logger.error("Error checking S3 object s3://%s/%s: %s", bucket, key, e)
            return False
    ...
    def purge_sqs_queue(queue_url: str, region: str = 'us-east-1') -> bool:
        ...
        try:
            ...
            logger.info("Purged queue: %s", queue_url)
            return True
        except ClientError as e:
