...
class ThingpressCleanup:
    ...
    def cleanup_all(self) -> Dict[str, Any]:
        ...
        """
        self.logger.info(f"🧹 Starting Thingpress cleanup (dry_run={self.config.dry_run})")
        self.logger.info(f"Region: {self.config.region}, Stack prefix: {self.config.stack_name_prefix}")
        ...
        try:
            ...
            if self.config.verify_cleanup and not self.config.dry_run:
                self.logger.info(f"⏳ Waiting {self.config.verification_wait_seconds}s for resources to fully delete...")
    ...
    def _cleanup_single_iot_thing(self, thing_name: str):
        ...
        try:
            self.logger.info(f"  📱 Cleaning up IoT thing: {thing_name}")
            
            if self.config.dry_run:
                self.logger.info(f"    [DRY RUN] Would delete thing: {thing_name}")
                return
            ...
            try:
                ...
                self.logger.warning(f"    Failed to get principals for {thing_name}: {e}")
    ...
    def _cleanup_certificate_from_thing(self, thing_name: str, principal_arn: str):
        ...
        try:
            ...
            try:
                ...
                for policy in policies_response.get('policies', []):
                    ...
                    try:
                        ...
                        self.logger.info(f"    🔓 Detached policy {policy_name} from certificate {cert_id}")
                    except ClientError as e:
                        self.logger.warning(f"    Failed to detach policy {policy_name}: {e}")
                        
            except ClientError as e:
                self.logger.warning(f"    Failed to list policies for certificate {cert_id}: {e}")
            
            # Detach certificate from thing
            try:
                self.iot_client.detach_thing_principal(thingName=thing_name, principal=principal_arn)
                self.logger.info(f"    🔗 Detached certificate {cert_id} from thing {thing_name}")
            except ClientError as e:
                self.logger.warning(f"    Failed to detach certificate {cert_id} from thing: {e}")
            ...
            try:
                ...
                self.logger.info(f"    🗑️ Deleted certificate: {cert_id}")
                
            except ClientError as e:
                self.logger.warning(f"    Failed to delete certificate {cert_id}: {e}")
                
        except Exception as e:
            self.logger.error(f"    Failed to cleanup certificate {principal_arn}: {e}")
    ...
    def _should_cleanup_s3_bucket(self, bucket_name: str) -> bool:
        ...
        try:
            ...
            if e.response['Error']['Code'] == 'NoSuchTagSet':
                ...
                self.logger.warning(f"Failed to get tags for bucket {bucket_name}: {e}")
                return False
    ...
    def _cleanup_single_s3_bucket(self, bucket_name: str):
        ...
        if self.config.dry_run:
            self.logger.info(f"    [DRY RUN] Would empty and prepare bucket: {bucket_name}")
            return
        ...
        try:
            ...
            self.logger.info(f"    ✅ Emptied S3 bucket: {bucket_name}")
    ...
    def _cleanup_single_stack(self, stack_name: str):
        ...
        self.logger.info(f"  🗂️ Cleaning up CloudFormation stack: {stack_name}")
        ...
        try:
            ...
            self.logger.info(f"    ✅ Deleted CloudFormation stack: {stack_name}")
    ...
    def _verify_cleanup(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.warning(f"Failed to verify IoT things cleanup: {e}")
        ...
        try:
            ...
            self.logger.warning(f"Failed to verify S3 buckets cleanup: {e}")
        ...
        if self.config.cleanup_cloudformation_stacks:
            try:
                ...
                self.logger.warning(f"Failed to verify CloudFormation stacks cleanup: {e}")
        
        # Log results
        if verification_results['issues_found'] == 0:
            self.logger.info("  🎉 Cleanup verification PASSED - All resources cleaned up successfully!")
            verification_results['success'] = True
        else:
            self.logger.error(f"  ❌ Cleanup verification FAILED - {verification_results['issues_found']} issues found")
            verification_results['success'] = False
            
            # Log remaining resources
            for resource_type, resources in verification_results['remaining_resources'].items():
                if resources:
                    self.logger.error(f"    Remaining {resource_type}: {resources}")
