"""
Generated Provider Component Integration Test

Tests the deployed Generated provider Lambda function through its interface:
1. Creates a test TXT manifest with generated certificates
2. Invokes the provider function with the test manifest
3. Verifies it processes the TXT format correctly
4. Checks that certificates are queued for bulk import
5. Validates the bulk importer processes the certificates
6. Verifies IoT things are created with proper configuration
"""
...
class GeneratedProviderComponentTest(ProviderComponentTest):
    """Component test for Generated provider"""
    
    def __init__(self):
        super().__init__('generated')
        self.test_manifest_path = project_root / 'test/artifacts/certificates_test.txt'
        
    def run_test(self) -> dict:
        """Run the complete Generated provider component test"""
        ...
        try:
            ...
            step2 = self.log_step("upload_manifest", "Upload Generated TXT manifest to S3")
            manifest_key = f"component-test/{self.test_id}/certificates.txt"
            ...
            step3 = self.log_step("invoke_provider", "Invoke Generated provider function")
            ...
            messages = self.wait_for_sqs_messages(bulk_queue_url, timeout_seconds=30, expected_count=1)
            ...
            # Step 5: Validate TXT parsing results
            step5 = self.log_step("validate_txt_parsing", "Validate TXT format was parsed correctly")
            
            # Check that certificates have expected Generated format
            if certificates:
                ...
                # Check for Generated-specific metadata
                if 'generated' in sample_cert or 'programmatic' in str(sample_cert).lower():
                    self.logger.info("Certificate appears to be programmatically generated")
            ...
            # For Generated certificates, thing names are typically derived from certificate CN
...
def run_generated_component_test():
    """Run the Generated provider component test"""
    test = GeneratedProviderComponentTest()
    results = test.run_test()
    
    # Print summary
    print("\n" + "="*60)
    print(f"🧪 GENERATED PROVIDER COMPONENT TEST RESULTS")
    print("="*60)
...
if __name__ == "__main__":
    success = run_generated_component_test()
