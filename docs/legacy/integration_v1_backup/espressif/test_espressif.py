...
def setup_test_environment():
    ...
    s3 = boto3.client('s3')
    manifest_path = os.path.join(os.path.dirname(__file__), '../../artifacts/manifest-espressif.csv')
    manifest_key = f"test-manifests/espressif-{test_id}.csv"
...
def run_espressif_test(env):
    """Run the Espressif provider test."""
    ...
    try:
        # Step 1: Trigger the Espressif provider
