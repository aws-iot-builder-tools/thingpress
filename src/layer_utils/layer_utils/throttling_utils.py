...
from .aws_utils import get_queue_depth, calculate_optimal_delay, send_sqs_message_batch_with_retry
...
class ThrottlingConfig:
    ...
    def __init__(self):
        self.auto_throttling_enabled = os.environ.get("AUTO_THROTTLING_ENABLED", "true").lower() == "true"
        ...
        self.use_adaptive_throttling = os.environ.get("USE_ADAPTIVE_THROTTLING", "false").lower() == "true"
...
class StandardizedThrottler:
    ...
    def apply_adaptive_throttling(self, queue_url: str, session: Session) -> None:
        ...
        try:
            ...
            delay = calculate_optimal_delay(queue_metrics['total'], self.config.throttling_base_delay)
