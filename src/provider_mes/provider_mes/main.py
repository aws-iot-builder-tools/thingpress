...
def validate_device_infos(data: dict) -> None:
    ...
    for idx, device in enumerate(data['devices']):
        ...
        if not re.match(r'^[a-fA-F0-9]{64}$', fingerprint):
            raise ValueError(f"Device {idx}: certFingerprint must contain only hex characters (0-9, a-f, A-F)")
...
def process_device_infos_file(config: dict, queue_url: str, session: Session = default_session) -> int:
    """
